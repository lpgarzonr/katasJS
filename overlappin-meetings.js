function mergeRanges(meetings) {
    ...
    const sortedMeetings = getSortedMeetings(meetings)
    let currentMergedRange = sortedMeetings[0]
    
    for (let i = 1; i < sortedMeetings.length; i++) {
      const mergedRange = getMergeRange(sortedMeetings[i], currentMergedRange)
      if (mergedRange) {
        currentMergedRange = mergedRange
      } else {
        mergedMeetings.push(currentMergedRange)
        currentMergedRange = sortedMeetings[i]
      }
      const isLastMeeting = i === sortedMeetings.length -1
      if(isLastMeeting) {
        mergedMeetings.push(currentMergedRange)
      }
    }
  ...
  function getSortedMeetings(meetings) {
    return meetings.sort((a, b) => a.startTime - b.startTime)
  }
  
  function getMergeRange(a, b) {
    const isOverlapping = a.startTime === b.startTime || 
    a.startTime < b.startTime && a.endTime >= b.startTime ||
    a.startTime > b.startTime && b.endTime >= a.startTime 
